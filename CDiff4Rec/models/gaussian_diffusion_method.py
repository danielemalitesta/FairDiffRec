...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
            steps, device, alpha = 0.5, random_seed = 1, history_num_per_term=10, beta_fixed=True):
    ...
    def get_output(self, model, x_start):
        batch_size, device = x_start.size(0), x_start.device # 400 x 2810
        ts, pt = self.sample_timesteps(batch_size, device, 'importance') # 1-D array like 400, 400
        ...
        target = {
            ModelMeanType.START_X: x_start, # 400 x 2810
            ModelMeanType.EPSILON: noise, # 400 x 2810
        }[self.mean_type]
        ...
        return target, model_output, ts, x_start, x_t, pt
    
    def get_losses(self, target, model_output, reweight, ts, x_start, x_t, pt):
        
        device = x_start.device
        terms = {}
        
        mse = mean_flat((target - model_output) ** 2) # 400 x 2810 -> 400 (1-D array) 

        if reweight == True:
            if self.mean_type == ModelMeanType.START_X:
                weight = self.SNR(ts - 1) - self.SNR(ts)
                weight = th.where((ts == 0), 1.0, weight) # timestep is propotional to weight
                loss = mse
    ...
    def training_losses(self, model, x_start, reweight=False, batch_sim = None):
        ...
        if batch_sim is not None:
            model_output = model_output.double()
            model_output = self.alpha * model_output + (1 - self.alpha) * (batch_sim @ model_output)
    ...
    def sample_timesteps(self, batch_size, device, method='uniform', uniform_prob=0.001):
        if method == 'importance':  # importance sampling
            ...
            if pt_all.sum(-1) - 1. > 1e-5:
                print("pt_all", pt_all)
                print("pt_all.sum(-1)", pt_all.sum(-1))
            ...
            t = th.randint(0, self.steps, (batch_size,), device=device).long() # 1-D array like 400
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        return (
            self._extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start # self._extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) = [[0.9996, 0.9996, 0.9996,  ..., 0.9996, 0.9996, 0.9996], ... , [0.9999, 0.9999, 0.9999,  ..., 0.9999, 0.9999, 0.9999]]
...
def mean_flat(tensor):
    ...
    """
    return tensor.mean(dim=list(range(1, len(tensor.shape)))) # batch 1-D array
