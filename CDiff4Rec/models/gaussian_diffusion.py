...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
            steps, device, random_seed = 1, history_num_per_term=10, beta_fixed=True):
    ...
    def p_sample(self, model, x_start, steps, sampling_noise=False):
        ...
        for i in indices:
            ...
            if sampling_noise:
                ...
                nonzero_mask = (
                    (t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1))) # 400 x 1 
                )  # no noise when t == 0
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        if reweight == True:
            if self.mean_type == ModelMeanType.START_X:
                ...
                weight = th.where((ts == 0), 1.0, weight) # timestep is propostional to weight
                loss = mse
