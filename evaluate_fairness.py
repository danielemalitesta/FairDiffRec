...
# Aggiungiamo la cartella CDiff4Rec al percorso di Python
sys.path.append('/content/FairDiffRec/CDiff4Rec')
import data_utils

# ================= IMPOSTAZIONI =================
K = 20
SH_RATIO = 0.2  # Il 20% degli item è considerato Short-Head (popolare)
TSV_PATH = '/content/FairDiffRec/best_recommendations_diffrec.tsv' # Il tuo file
DATA_DIR = '/content/FairDiffRec/datasets/ml-1m/'

print(f"=== INIZIO VALUTAZIONE FAIRNESS @{K} ===\n")

# 1. Caricamento Dati e Matrici
print("1. Caricamento dei file e delle matrici...")
...
# 2. Mappatura Utenti e Gruppi Demografici (Gender)
...
# 3. Mappatura Popolarità Item (Short-Head vs Long-Tail)
print("2. Suddivisione Item Popolari (SH) e Nicchia (LT)...")
...
# 4. Calcolo metriche base per Utente
print("3. Calcolo metriche per utente...")
...
# 5. Calcolo Consumer Fairness
def get_delta(metric_array):
    # Considera solo gli utenti che hanno effettivamente item nel test set
...
# 6. Calcolo Provider Fairness
print("4. Calcolo Provider Fairness...")
...
# ================= RISULTATI =================
print("\n" + "="*45)
print(f" RISULTATI FAIRNESS (K={K})")
print("="*45)
print(f"[Consumer Fairness]")
print(f" - Differenza tra gruppi demografici: {genders[0]} vs {genders[1]}")
print(f" - Delta Recall:   {delta_rec:.5f}")
print(f" - Delta NDCG:     {delta_ndcg:.5f}")
print(f"\n[Provider Fairness]")
print(f" - Delta Exposure: {delta_exp:.5f}")
print(f" - APLT:           {aplt:.5f} ({(aplt*100):.2f}% degli item raccomandati sono di nicchia)")
print("="*45)